...
Hermes API must remain private and authenticated. The daily target is direct HTTPS through `https://api.hermes.example.com`; local SSH tunnel mode is a fallback only. See `docs/operations/HERMES_DIRECT_API.md`.
