...
SkyBridge APIs are local-first and return safe derived metadata. They must not expose raw prompts, patches, stdout, stderr, tokens, cookies or production secrets.
...
- `GET /v1/hermes/summary`: private Hermes tunnel/API/capability status from local telemetry.
...
The product summary APIs are intended for dashboard and embed consumption. Field names use snake_case to match existing server responses. Derived PR, Hermes and auto-merge summaries are local/fixture-backed unless future integrations explicitly persist richer state.
