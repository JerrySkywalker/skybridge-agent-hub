...
- [x] Add manifest-gated CI artifact upload for fixture-only screenshots when Playwright is available.

## Completion Criteria

- Screenshot artifacts and a fixture-only `manifest.json` are produced locally or in CI for review. Local artifact generation is implemented when Playwright is installed; CI invokes the skip-safe runner, uploads its sanitized log, and uploads screenshots only when the fixture-only manifest is present and validated.
