...
$TargetTaskId = "live-codex-analysis-report-task-337-001"
...
$CreateConfirmationPhrase = "I_UNDERSTAND_CREATE_ONE_LIVE_CODEX_ANALYSIS_REPORT_TASK_ONLY"
$RunConfirmationPhrase = "I_UNDERSTAND_CLAIM_AND_RUN_ONE_LIVE_CODEX_ANALYSIS_REPORT_TASK_ONLY"
...
function Test-TaskHasCodexReportMetadata {
  ...
  (
    [string](Get-PropertyValue $metadata "adapter") -eq "mg337-codex-analysis-report" -and
    [string](Get-PropertyValue $metadata "reason") -eq "mg337_one_live_codex_analysis_report_trial" -and
    [string](Get-PropertyValue $metadata "source_run_id") -eq "mega-goal-337-codex-analysis-report-golden-trial" -and
...
function New-CodexReportTaskPayload {
  [pscustomobject]@{
    ...
    title = "MG337 Codex analysis report golden trial"
    ...
    prompt_summary = "MG337 deterministic Codex analysis report. Safe summary/metrics input only; no raw prompt or logs persisted."
    risk = "medium"
    source = "manual"
    ...
    validation = @(
      ...
      "report.md exists under .agent/tmp/codex-analysis-report",
      "report states synthetic runner validation",
      "server evidence lists only actual report file",
    ...
    planner_metadata = @{
      adapter = "mg337-codex-analysis-report"
      decision = "continue"
      reason = "mg337_one_live_codex_analysis_report_trial"
      task_type = "codex-analysis-report"
      ...
      validation = @(
        "fixed Codex analysis report runner completed",
        "report.md exists under allowed path",
        "raw logs and raw prompt are not included",
        "token_printed=false"
      )
      expected_outputs = @($OutputReport)
      stop_criteria_status = @("complete_exact_live_codex_analysis_report_trial_then_stop")
      source_run_id = "mega-goal-337-codex-analysis-report-golden-trial"
...
function Invoke-CreatePreview {
  ...
  if ($targetTaskId -ne $TargetTaskId) { $blockers.Add("unexpected_live_codex_analysis_report_task_id") | Out-Null }
  ...
  if ($existing -and -not (Test-TaskHasCodexReportMetadata $existing)) { $blockers.Add("existing_task_not_mg337_codex_report") | Out-Null }
  if ($existing -and [string]$existing.status -notin @("queued", "completed", "failed")) { $blockers.Add("existing_task_not_safe_state") | Out-Null }
  [pscustomobject]@{
    schema = "skybridge.live_codex_analysis_report_create_preview.v1"
...
function Invoke-CreateApply {
  ...
  if (-not $preview.ok) {
    return [pscustomobject]@{
      schema = "skybridge.live_codex_analysis_report_create_result.v1"
      ok = $false
      mode = "apply"
      project_id = $ProjectId
  ...
  if ($preview.task_exists) {
    return [pscustomobject]@{
      ...
      review_reason = "existing_mg337_codex_report_task_reused_without_requeue"
      blockers = @()
      claim_created = $false
      execution_started = $false
      matlab_run_called = $false
  ...
  [pscustomobject]@{
    ...
    review_reason = if ($created) { "exact_confirmation_received_created_one_live_codex_analysis_report_task" } else { "task_create_failed" }
...
function Test-RunEligibility {
  ...
  if ($TaskId -ne $TargetTaskId) { $reasons.Add("unexpected_live_codex_analysis_report_task_id") | Out-Null }
  ...
  if ($Task) {
    ...
    if ($template -ne $TemplateIdTarget) { $reasons.Add("template_not_supported_mg337_codex_report") | Out-Null }
    if ($runner -ne $RunnerIdTarget) { $reasons.Add("runner_not_supported_mg337_codex_report") | Out-Null }
    ...
    if (-not (Test-TaskHasCodexReportMetadata $Task)) { $reasons.Add("task_not_created_by_mg337_codex_report") | Out-Null }
  ...
  if ($RunnerPreview.codex_available -ne $true) { $reasons.Add("codex_not_available") | Out-Null }
  [pscustomobject]@{
    task = $Task
...
function New-RunRecord {
  ...
  [pscustomobject]@{
    schema = if ($Mode -eq "preview") { "skybridge.live_codex_analysis_report_run_preview.v1" } else { "skybridge.live_codex_analysis_report_run_result.v1" }
    ok = $Ok
    mode = $Mode
    worker_id = $WorkerId
    project_id = $ProjectId
    ...
    result_summary = if (-not [string]::IsNullOrWhiteSpace($ResultSummary)) { $ResultSummary } elseif ($Ok) { "MG337 exact Codex report task is eligible for fixed-runner apply." } else { "MG337 Codex report task is not eligible for live apply." }
...
function Invoke-RunPreview {
  if ($TaskId -ne $TargetTaskId) {
    $dummy = [pscustomobject]@{
      eligible = $false
      rejected_reason = "unexpected_live_codex_analysis_report_task_id"
...
function Get-FinishBody {
  ...
  @{
    ...
    evidence_summary = @{
      ...
      risk_status = "medium_fixed_codex_analysis_report_trial"
      summary = [string]$Evidence.result_summary
      raw_codex_log_included = $false
      raw_prompt_included = $false
      raw_stdout_included = $false
...
function Invoke-RunApply {
  ...
  if (-not $Confirm -or $ConfirmationText -ne $RunConfirmationPhrase) {
    ...
    return New-RunRecord -Mode apply -Ok $false -Eligibility $preview -ResultSummary "Exact MG337 run confirmation is required before claim/start/Codex report apply."
  }
  ...
  if (-not $preview.ok -and [string]$preview.rejected_reason -match "worker_not_online|worker_not_registered_or_offline") {
    ...
    if (-not $postHeartbeat.eligible) {
      return New-RunRecord -Mode apply -Ok $false -Eligibility $postHeartbeat -HeartbeatRefresh $heartbeat -ResultSummary "Worker heartbeat refresh did not leave the exact MG337 task eligible; no claim was created."
    }
...
function Get-Status {
  ...
  $runnerStatus = Invoke-Runner -RunnerCommand "status"
  [pscustomobject]@{
    schema = "skybridge.live_codex_analysis_report_status.v1"
...
function Get-Report {
  ...
  [pscustomobject]@{
    schema = "skybridge.live_codex_analysis_report_report.v1"
...
if ($Command -eq "status") {
  ...
  $result = [pscustomobject]@{
    schema = "skybridge.live_codex_analysis_report_safe_summary.v1"
    ok = $true
    worker_id = $WorkerId
    task_id = $TaskId
    template_id = $TemplateIdTarget
    runner_id = $RunnerIdTarget
    next_safe_action = "preview_create_then_exact_confirm_create_then_preview_run_then_exact_confirm_run_one_codex_report_task"
    claim_created = $false
    execution_started = $false
    matlab_run_called = $false
    worker_loop_started = $false
  ...
  $result = [pscustomobject]@{
    schema = "skybridge.live_codex_analysis_report_apply_run.v1"
    ok = $false
