...
function Invoke-MatlabGoldenTrial {
  param([string]$Command)
  $scriptPath = Join-Path $PSScriptRoot "skybridge-live-matlab-golden-trial.ps1"
  $raw = & pwsh -NoProfile -ExecutionPolicy Bypass -File $scriptPath `
    ...
    -TaskId "live-matlab-golden-task-333-001" `
...
try {
  ...
  Invoke-WorkerTemplateRunnerJson "POST" "/v1/projects" @{
    ...
    name = "MG333 MATLAB Golden Trial Preview Fixture"
  } | Out-Null
  Invoke-WorkerTemplateRunnerJson "POST" "/v1/workers/register" @{
    ...
    labels = @("mg333-fixture", "matlab-golden")
    enabled = $true
  } | Out-Null
  Invoke-WorkerTemplateRunnerJson "POST" "/v1/workers/jerry-win-local-01/heartbeat" @{
    status_note = "mg333 preview fixture ready"
    load = 0
  } | Out-Null

  $status = Invoke-MatlabGoldenTrial -Command "status"
  if ([string]$status.schema -ne "skybridge.live_matlab_golden_trial_status.v1") { throw "Unexpected status schema." }
  ...
  $previewCreate = Invoke-MatlabGoldenTrial -Command "preview-create"
  if ([string]$previewCreate.schema -ne "skybridge.live_matlab_golden_trial_create_preview.v1") { throw "Unexpected create preview schema." }
  ...
  $previewRun = Invoke-MatlabGoldenTrial -Command "preview-run"
  if ([string]$previewRun.schema -ne "skybridge.live_matlab_golden_trial_run_preview.v1") { throw "Unexpected run preview schema." }
