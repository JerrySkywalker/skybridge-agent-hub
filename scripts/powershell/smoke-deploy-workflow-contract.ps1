...
foreach ($needle in @("workflow_run:", "workflows: [""Docker Images""]", "github.event.workflow_run.conclusion == 'success'", "github.event.workflow_run.head_branch == 'main'", "workflow_dispatch:", "missing_required_secrets", "cloud-deploy-report", "deploy-skybridge-server.sh", 'owner="${GITHUB_REPOSITORY_OWNER,,}"', 'image_ref="ghcr.io/$owner/skybridge-agent-hub-server:$expected_tag"')) {
