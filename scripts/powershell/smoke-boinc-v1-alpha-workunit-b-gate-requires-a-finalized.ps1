...
$evidenceDir = Join-Path ([System.IO.Path]::GetTempPath()) ("skybridge-alpha-gate-" + [guid]::NewGuid().ToString("N"))
$json = (& pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File (Join-Path $PSScriptRoot "skybridge-boinc-v1-alpha.ps1") -Command workunit-b-apply-gate -AuthorizeGoal216 -EvidenceDir $evidenceDir -Json | Out-String).Trim() | ConvertFrom-Json
