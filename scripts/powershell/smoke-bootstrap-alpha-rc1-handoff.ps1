...
$result = Invoke-JsonScript "skybridge-bootstrap-alpha-rc1-handoff.ps1" @("-Command", "status")

if ([string]$result.schema -ne "skybridge.bootstrap_alpha_rc1_handoff.v1") { throw "Unexpected RC1 handoff schema." }
if ([string]$result.status -notin @("pass", "warning")) { throw "RC1 handoff status should be pass or warning." }
