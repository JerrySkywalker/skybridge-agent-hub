...
foreach ($forbidden in @("start-one -Apply", "start-all -Apply", "resume -Apply", "execute-step", "worker loop started")) {
  if (($web + "`n" + $desktop) -match [regex]::Escape($forbidden)) { throw "Attention UI contains execution control text: $forbidden" }
}
foreach ($required in @("start_one_apply", "start_queue_apply", "start_all", "no_execution_enablement_in_goal_194")) {
