...
foreach ($required in @("Controlled Review / Import", "Approve preview", "Reject preview", "Edit staged", "Import preview", "Import apply disabled", "reason-gated", "manifest diff", "execution review required")) {
  if ($ui -notmatch [regex]::Escape($required) -and $client -notmatch [regex]::Escape($required)) { throw "Desktop review/import surface missing: $required" }
}
foreach ($forbidden in @("Start imported goal", "Execute imported goal", "start-all -Apply", "resume -Apply")) {
  if ($ui -match [regex]::Escape($forbidden)) { throw "Desktop contains forbidden execution control: $forbidden" }
}
$summary = [pscustomobject]@{ ok = $true; scenario = "desktop-goal-draft-review-import"; token_printed = $false }
