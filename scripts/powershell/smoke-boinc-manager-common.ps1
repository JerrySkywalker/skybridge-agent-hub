...
function Invoke-BoincManagerJson {
  param([Parameter(Mandatory = $true)][string]$Command)
  $scriptPath = Join-Path $PSScriptRoot "skybridge-boinc-manager.ps1"
  $raw = & pwsh -NoProfile -ExecutionPolicy Bypass -File $scriptPath -Command $Command -Json
