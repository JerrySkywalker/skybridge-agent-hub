...
if ($result.report.current_goal_id -ne "super-191-readonly-operator-dashboard") { throw "Expected Goal 191 current goal after Goal 190 completion." }
if ($result.report.current_goal_status -ne "ready") { throw "Expected Goal 191 ready." }
if ($result.report.current_goal_unexecuted -ne $true) { throw "Expected Goal 191 unexecuted." }
if (@($current.linked_task_ids).Count -ne 0) { throw "Expected Goal 191 linked task ids empty." }
if (@($current.linked_pr_urls).Count -ne 0) { throw "Expected Goal 191 linked PR URLs empty." }
if ($current.evidence_status -ne "missing") { throw "Expected Goal 191 evidence to remain missing before this PR is attached." }
