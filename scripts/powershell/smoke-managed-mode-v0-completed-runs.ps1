...
if ($archive.completed_run_count -ne 3) { throw "Expected exactly three completed runs." }
foreach ($id in @("managed-mode-pilot-208", "managed-mode-run-209", "managed-mode-run-210")) {
  if (@($archive.completed_run_ids) -notcontains $id) { throw "Missing completed run $id." }
}
foreach ($file in @("docs/managed-mode-pilot-orientation.md", "docs/managed-mode-repeatability-orientation.md", "docs/managed-mode-v0-operator-checklist.md")) {
