...
foreach ($required in @("BOINC v1 alpha completed", "Alpha completed", "Workunit C absent", "Next safe action", "token_printed=false")) {
  if ($text -notlike "*$required*") { throw "missing web completed alpha marker: $required" }
}
[pscustomobject]@{ ok = $true; scenario = "web-boinc-v1-alpha-completed-panel"; token_printed = $false } | ConvertTo-Json -Compress
