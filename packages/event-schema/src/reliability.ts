...
export const ServerApprovedWorkunitBlockerSchema = z.object({
  schema: z.literal("skybridge.server_approved_workunit_blocker.v1"),
  blocker_id: z.string().min(1),
  reason: z.string().min(1),
  token_printed: z.literal(false),
});

export const ServerApprovedWorkunitPolicySchema = z.object({
  schema: z.literal("skybridge.server_approved_workunit_policy.v1"),
  mode: z.literal("server_approved_one_workunit"),
  run_id: z.literal("server-approved-run-225"),
  max_workunits: z.literal(1),
  max_tasks: z.literal(1),
  max_claims: z.literal(1),
  max_codex_executions: z.literal(1),
  ...
  allowed_paths: z.array(z.string()),
  require_release_gate: z.literal(true),
  require_pairing_gate: z.literal(true),
  require_approval_gate: z.literal(true),
  require_resident_polling_gate: z.literal(true),
  require_resource_gate: z.literal(true),
  require_failure_budget: z.literal(true),
  require_evidence_retention: z.literal(true),
  require_audit_redaction: z.literal(true),
  require_safe_export: z.literal(true),
  require_human_review: z.literal(true),
  require_finalizer: z.literal(true),
  remote_execution_enabled: z.literal(false),
  arbitrary_command_enabled: z.literal(false),
  generic_queue_apply_enabled: z.literal(false),
  trusted_docs_auto_merge_enabled: z.literal(false),
  stop_on_pr_created: z.literal(true),
  stop_on_ci_failure: z.literal(true),
  stop_on_warning: z.literal(true),
  token_printed: z.literal(false),
});

export const ServerApprovedWorkunitSchema = z.object({
  schema: z.literal("skybridge.server_approved_workunit.v1"),
  run_id: z.literal("server-approved-run-225"),
  workunit_id: z.literal("server-approved-run-225-workunit-001"),
  task_id: z.literal("server-approved-run-225-task-001"),
  worker_id: z.string().min(1),
  task_type: z.literal("docs/local-smoke"),
  risk: z.literal("low"),
  target_path: z.literal("docs/server-approved-workunit-225.md"),
  allowed_paths: z.array(z.string()),
  release_gate: z.literal("required"),
  pairing_gate: z.literal("required"),
  approval_gate: z.literal("required"),
  resident_polling_gate: z.literal("required"),
  resource_gate: z.literal("required"),
  failure_budget: z.literal("required"),
  evidence_retention: z.literal("required"),
  audit_redaction: z.literal("required"),
  safe_export: z.literal("required"),
  human_review: z.literal("required"),
  finalizer: z.literal("required"),
  max_workunits: z.literal(1),
  max_tasks: z.literal(1),
...
export const ServerApprovedWorkunitGateSchema = z.object({
  schema: z.literal("skybridge.server_approved_workunit_gate.v1"),
  run_id: z.literal("server-approved-run-225"),
  workunit_id: z.literal("server-approved-run-225-workunit-001"),
  task_id: z.literal("server-approved-run-225-task-001"),
  gate_result: z.enum(["pass", "blocked"]),
  ...
  max_task_prs: z.literal(1),
  release_gate_result: z.enum(["pass", "blocked"]),
  pairing_gate_result: z.enum(["pass", "blocked"]),
  approval_gate_result: z.enum(["pass", "blocked"]),
  approval_consumption_status: z.enum(["not_consumed", "consumed", "rejected"]),
  resident_polling_gate_result: z.enum(["pass", "blocked"]),
  resource_gate_result: z.enum(["pass", "blocked"]),
  failure_budget_gate_result: z.enum(["pass", "blocked"]),
  evidence_retention_gate_result: z.enum(["pass", "blocked"]),
  audit_redaction_gate_result: z.enum(["pass", "blocked"]),
  safe_export_gate_result: z.enum(["pass", "blocked"]),
  resident_polling_bridge: z.record(z.unknown()),
  active_tasks: z.literal(0),
  stale_leases: z.literal(0),
  runner_lock: z.literal("none"),
  open_task_pr_count: z.number().int().nonnegative(),
  no_next_execution_authorized: z.literal(true),
  blockers: z.array(ServerApprovedWorkunitBlockerSchema),
  token_printed: z.literal(false),
});

export const ServerApprovedExecutionBoundarySchema = z.object({
  schema: z.literal("skybridge.server_approved_execution_boundary.v1"),
  run_id: z.literal("server-approved-run-225"),
  remote_execution_enabled: z.literal(false),
  arbitrary_command_enabled: z.literal(false),
  generic_queue_apply_enabled: z.literal(false),
...
export const ServerApprovedWorkunitStatusSchema = z.object({
  schema: z.literal("skybridge.server_approved_workunit_status.v1"),
  run_id: z.literal("server-approved-run-225"),
  workunit: ServerApprovedWorkunitSchema,
  policy: ServerApprovedWorkunitPolicySchema,
  gate: ServerApprovedWorkunitGateSchema,
  execution_boundary: ServerApprovedExecutionBoundarySchema,
  finalizer_preview: z.record(z.unknown()),
  token_printed: z.literal(false),
});
...
export type ServerApprovedWorkunitStatus = z.infer<typeof ServerApprovedWorkunitStatusSchema>;
...
export const fixtureServerApprovedWorkunitStatus: ServerApprovedWorkunitStatus = {
  schema: "skybridge.server_approved_workunit_status.v1",
  run_id: "server-approved-run-225",
  workunit: {
  ...
  finalizer_preview: {
    ...
    can_apply: false,
    task_pr_url: "https://github.com/JerrySkywalker/skybridge-agent-hub/pull/170",
    finalizer_evidence_path: ".agent/tmp/server-approved-run-225/finalizer-evidence.json",
    human_review_required: true,
    no_auto_merge: true,
    no_raw_artifacts: true,
    token_printed: false,
...
export function parseServerApprovedWorkunitStatus(input: unknown): ServerApprovedWorkunitStatus {
  return ServerApprovedWorkunitStatusSchema.parse(input);
}
